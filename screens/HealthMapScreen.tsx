...
export default function HealthMapScreen() {
  ...
  const getMarkerColor = (vaccinationCoverage: number) => {
    if (vaccinationCoverage >= 90) return '#A78BFA'; // secondary (purple)
    if (vaccinationCoverage >= 85) return '#FCD34D'; // accent (yellow)
          ...
          {showLegend && (
            <View className="absolute bottom-4 right-4 rounded-lg bg-slate-900/95 p-3">
              ...
              <View className="flex-row items-center gap-2">
                <View className="h-3 w-3 rounded-full" style={{ backgroundColor: '#A78BFA' }} />
        ...
        <ScrollView className="flex-1 px-4 pt-4" contentContainerStyle={{ paddingBottom: 20 }}>
            ...
            <View className="gap-3">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-slate-200">
                  Total Population:{' '}
                  <Text className="font-semibold">{cityStats.population.toLocaleString()}</Text>
                </Text>
              </View>
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-slate-200">
                  Children Monitored:{' '}
                  <Text className="font-semibold">{cityStats.totalChildren.toLocaleString()}</Text>
                </Text>
              </View>
              <View className="flex-row items-center gap-2">
          ...
          <View className="flex-row gap-2">
            <View className="flex-1 rounded-lg bg-slate-800 p-4">
              <Text className="text-2xl font-bold" style={{ color: '#A78BFA' }}>
                {cityStats.averageVaccinationRate}%
              </Text>
              <Text className="text-xs text-slate-300">Avg. Vaccination Rate</Text>
            </View>
            <View className="flex-1 rounded-lg bg-slate-800 p-4">
              <Text className="text-2xl font-bold" style={{ color: '#FF6B35' }}>
          ...
          <View className="rounded-lg border border-secondary/50 bg-secondary/20 p-3">
            <Text className="text-sm" style={{ color: '#A78BFA' }}>
              <MapPin size={14} color="#A78BFA" /> Tap any marker on the map to view detailed
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {selectedBarangay && (
                <View className="gap-4">
                  ...
                  <View className="rounded-lg bg-slate-800 p-4">
                    <View className="mb-3 flex-row items-center gap-2">
                      <HeartPulse size={18} color="#FCD34D" />
                    ...
                    <View className="gap-2">
                      ...
                      <View className="flex-row justify-between">
                        ...
                        <Text className="font-semibold" style={{ color: '#FF6B35' }}>
                      ...
                      <View className="flex-row justify-between">
                        ...
                        <Text className="font-semibold" style={{ color: '#FF6B35' }}>
                          {selectedBarangay.maternalHealth.prenatalVisitCompliance}%
                        </Text>
                      </View>
                    </View>
                  </View>
